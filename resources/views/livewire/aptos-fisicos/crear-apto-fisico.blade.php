    ...
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            ...
            <h2 class="text-3xl font-black text-slate-900 dark:text-white tracking-tight">
                Registrar Apto Físico: <span class="text-indigo-600 dark:text-indigo-400">{{ $socio->nombre }} {{ $socio->apellido }}</span>
            </h2>
            <p class="text-sm text-slate-500 dark:text-slate-300 mt-1 font-semibold">
                Suba el certificado médico firmado y registre su periodo de vigencia.
            </p>
        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input 
                                ...
                                class="form-input !font-semibold"
                            >
                            <span class="text-[10px] text-slate-500 dark:text-slate-300 mt-1.5 block font-semibold">Por defecto se calcula a +1 año desde la fecha de emisión, pero se puede modificar manualmente.</span>
                            @error('fecha_vencimiento') <span class="text-xs text-rose-500 font-semibold mt-1 block">{{ $message }}</span> @enderror
                        </div>

                        <!-- Subida de Archivo -->
                        <div class="col-span-1 md:col-span-2">
                            <label for="archivo_cargado" class="form-label">Certificado Médico (opcional)</label>
                            ...
                            <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-300 dark:border-slate-700 border-dashed rounded-xl hover:border-indigo-500 transition duration-150 bg-slate-50 dark:bg-slate-900/50">
                                <div class="space-y-1 text-center">
                                    ...
                                    <p class="text-xs text-slate-500 dark:text-slate-300 font-semibold">PDF, PNG, JPG, JPEG hasta 4MB</p>
                                </div>
                            </div>
                            ...
                            @if ($archivo_cargado)
                                <div class="mt-2 text-sm text-emerald-600 dark:text-emerald-400 font-bold flex items-center gap-1">
                                    ...
                                    Archivo cargado temporalmente: {{ $archivo_cargado->getClientOriginalName() }}
                                </div>
                            @endif
